...
public:
	...
	class ChannelReader {
		using CSample = typename std::conditional<isConst, const float, float>::type;
		CSample *data;
		int stride;
	public:
	...
	Wav(unsigned int sampleRate, int channels, const std::vector<float> &samples) : sampleRate(sampleRate), channels(channels), samples(samples) {}
			...
			if (!hasFormat && blockType == value_fmt) {
				...
				case Format::PCM:
					...
					for (size_t i = 0; i < blockLength/2; ++i) {
						...
						if (file.eof()) break;
						if (value >= 32768) {
							samples.push_back(((float)value - 65536)/32768);
						} else {
							samples.push_back((float)value/32768);
						}
					}
				}
	...
	void makeMono() {
		std::vector<float> newSamples(samples.size()/channels, 0);
